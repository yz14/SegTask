...
class GPUAugmentor:
    ...
    def __call__(
        ...
        image, weight_map, cond = _random_flip(
            ...
            weight_map=weight_map, cond=cond)
...
def _grid_dropout(
    image: torch.Tensor,
    prob: float, ratio: float, num_holes: int) -> torch.Tensor:
    ...
    selected = _bernoulli_mask(B, prob)
    if not selected.any():
        return image
...
def _gaussian_blur_3d(
    ...
    sigmas = torch.empty(n, dtype=image.dtype).uniform_(
        sigma_range[0], sigma_range[1]).to(device, non_blocking=True)
