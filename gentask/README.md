...
- **显存与多卡**：`model.grad_checkpointing` UNet 系与 ADM/EDM2（含扩散 backbone）均支持（edsr/rcan 开启时 warning 提示忽略）；`train.gpus` 配多卡即启用 DDP（mp.spawn 每卡一进程，PSNR/SSIM 加权跨卡归约，落盘仅 rank0），单卡/CPU 路径零变化。
