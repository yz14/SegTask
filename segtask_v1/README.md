...
segtask_v1 是当前活跃的 2.5D / 3D 医学图像分割工程：`train.py` 负责训练，`predict.py` 负责滑窗推理，`config.py` 负责 YAML + dataclass 配置。它保留 3D `z_axis` / `cubic` / `whole` 三种 patch 模式与 2.5D 单视图管线，支持 NIfTI 或预打包 npz、bbox 裁剪、标签 one-vs-rest、可选多分辨率 / 多视图上下文、GPU 增强与多解码器组合（UNet / UNet++ / UNet3+，以及 ResNet / ConvNeXt / MedNeXt 编码骨干）。当前主线是 `segtask_v1/`；根目录 `segtask/` 是早期 v0 原型，已冻结仅供参考。
...
```text
segtask_v1/
├── segtask_v1/__init__.py  # 包入口 / 对外导出
├── segtask_v1/__main__.py  # 模块 CLI 入口
├── segtask_v1/config.py  # @dataclass YAML 配置系统
├── segtask_v1/logging_utils.py  # 日志初始化与控制台格式
├── segtask_v1/predict.py  # 预测入口
├── segtask_v1/train.py  # 训练入口
├── segtask_v1/utils.py  # 通用工具与小型指标
├── segtask_v1/data/__init__.py  # 包入口 / 对外导出
├── segtask_v1/data/augment.py  # GPUAugmentor：共享 grid_sample 的增强入口
├── segtask_v1/data/dataset.py  # NIfTI/npz IO、patch 抽取、bbox、3 个 Dataset
├── segtask_v1/data/loader.py  # 样本发现、配对、切分、dataloader 工厂
├── segtask_v1/data/make_data.py  # 离线把 NIfTI 烘焙成 bbox-cropped npz
├── segtask_v1/data/mixed_sampler.py  # 金/粗标混合采样器
├── segtask_v1/data/specs.py  # patch_mode → DatasetSpec 策略对象
├── segtask_v1/launcher/__init__.py  # 包入口 / 对外导出
├── segtask_v1/launcher/__main__.py  # 模块 CLI 入口
├── segtask_v1/launcher/assets.py  # 网页启动器内嵌静态资源
├── segtask_v1/launcher/build.py  # 启动器构建 / 打包逻辑
├── segtask_v1/launcher/manifest.py  # 任务清单与页面描述
├── segtask_v1/launcher/page.py  # 页面结构与视图拼装
├── segtask_v1/launcher/process.py  # 训练/推理进程管理
├── segtask_v1/launcher/schema.py  # 启动器数据 schema
├── segtask_v1/launcher/server.py  # 本地 HTTP 服务入口
├── segtask_v1/losses/__init__.py  # 包入口 / 对外导出
├── segtask_v1/losses/losses.py  # 基础损失与组合包装器
├── segtask_v1/losses/topo_aux.py  # 拓扑辅助损失
├── segtask_v1/models/__init__.py  # 包入口 / 对外导出
├── segtask_v1/models/adm_unet.py  # ADM 风格 U-Net 实现
├── segtask_v1/models/blocks.py  # 共享卷积 / 归一化 / 上下采样 / 注意力积木
├── segtask_v1/models/convnext.py  # ConvNeXt 块与 stage
├── segtask_v1/models/edm2_unet.py  # EDM2 风格 U-Net 实现
├── segtask_v1/models/factory.py  # 模型装配工厂
├── segtask_v1/models/mednext.py  # MedNeXt 残差倒瓶颈块
├── segtask_v1/models/resnet.py  # 2D/3D ResNet 残差块与 stage
├── segtask_v1/models/stem.py  # stem 与多视图融合策略
├── segtask_v1/models/topology.py  # 派生几何 / 通道真相源
├── segtask_v1/models/unet.py  # 通用 UNet / Encoder 主体
├── segtask_v1/models/unet3p.py  # UNet3+ decoder
├── segtask_v1/models/unetpp.py  # UNet++ nested decoder
├── segtask_v1/monitor/__init__.py  # 包入口 / 对外导出
├── segtask_v1/monitor/__main__.py  # 模块 CLI 入口
├── segtask_v1/monitor/assets.py  # 监控页内嵌样式与 SVG
├── segtask_v1/monitor/charts.py  # 指标聚合与图表数据
├── segtask_v1/monitor/dashboard.py  # 历史指标 → 自包含 HTML
├── segtask_v1/monitor/history.py  # 日志落盘与历史读取
├── segtask_v1/predictor/__init__.py  # 包入口 / 对外导出
├── segtask_v1/predictor/adabn.py  # AdaBN 自适应批归一化
├── segtask_v1/predictor/blending.py  # 滑窗融合与几何 helpers
├── segtask_v1/predictor/forwards.py  # forward / TTA / 诊断
├── segtask_v1/predictor/inputs.py  # window / batch 构造
├── segtask_v1/predictor/io.py  # run_inference 与 ckpt / precision
├── segtask_v1/predictor/predictor.py  # 预测器主类
├── segtask_v1/predictor/sliding.py  # 多种滑窗主循环
├── segtask_v1/trainer/__init__.py  # 包入口 / 对外导出
├── segtask_v1/trainer/amp.py  # AMP dtype / GradScaler 工具
├── segtask_v1/trainer/breakdown.py  # loss 分解与统计
├── segtask_v1/trainer/checkpoint.py  # checkpoint 读写与兼容
├── segtask_v1/trainer/dist_utils.py  # 分布式辅助函数
├── segtask_v1/trainer/memory.py  # 显存统计工具
├── segtask_v1/trainer/optim.py  # optimizer / scheduler / warmup
├── segtask_v1/trainer/trainer.py  # 训练循环主类
├── segtask_v1/trainer/validation.py  # 验证逻辑与指标汇总
├── segtask_v1/trainer/views.py  # 多视图切分与还原
├── segtask_v1/trainer/pipelines/__init__.py  # 包入口 / 对外导出
├── segtask_v1/trainer/pipelines/base.py  # 模块文件
├── segtask_v1/trainer/pipelines/factory.py  # 模块文件
├── segtask_v1/trainer/pipelines/lift25d.py  # 模块文件
├── segtask_v1/trainer/pipelines/patch3d.py  # 模块文件
├── segtask_v1/trainer/pipelines/slab25d.py  # 模块文件
├── segtask_v1/trainer/pipelines/vanilla3d.py  # 模块文件
├── segtask_v1/visualization/__init__.py  # 包入口 / 对外导出
├── segtask_v1/visualization/data_flow.py  # 数据流可视化 builder
├── segtask_v1/visualization/graph.py  # 可视化 IR / 图结构
├── segtask_v1/visualization/model_flow.py  # 模型流可视化 builder
├── segtask_v1/visualization/predict_flow.py  # 预测流可视化 builder
└── segtask_v1/visualization/render.py  # IR → 自包含 HTML 渲染器
```
