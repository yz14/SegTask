...
class Slab2_5DAuxPipeline(ViewPipeline):
    ...
    def compute_loss(self, pred, sup: SupervisionPack, breakdown=None):
        ...
        heads = [("main", accumulate_main(self.criterion, main_pred, sup))]  # 主路损失，包含deep supervision
...
class Slab2_5DNativeDPipeline(ViewPipeline):
    ...
    def compute_loss(self, pred, sup: SupervisionPack, breakdown=None):
        ...
        heads = [("main", accumulate_main(self.criterion, main_pred, sup))]
        if aux_preds and self.aux_loss_fns:
            ...
            for k_idx, (ap, loss_k, lbl_k) in enumerate(zip(
                    aux_preds, self.aux_loss_fns, sup.aux_labels)):
                view_k = k_idx + 1
