    ...
    def __init__(
        ...
        if in_ch_per_view_list is not None:  # 2.5D多分辨率输入
            ...
            if self.recon_in_channels % num_stem_fusion_views != 0:
                raise ValueError(
                    f"recon_in_channels ({self.recon_in_channels}) must be "
                    f"divisible by num_stem_fusion_views "
                    f"({num_stem_fusion_views})")
        ...
        if self.cond_in_channels > 0:
            ...
            if cond_stride != self.stem_stride:
                raise RuntimeError(
                    f"cond stem stride {cond_stride} != main stem stride "
                    f"{self.stem_stride}.")
