...
class ModelTopology:
    ...
    cond_in_channels     : int = 0      # 条件输入通道数（生成 conditioning；无 cond 恒 0）
...
def build_topology(cfg: "Config") -> ModelTopology:
    ...
    # 条件输入（生成任务 cond_dirs）：2.5D 逐 slice 折叠 D 到通道。分割 Config 尚无
    # cond_dirs 字段（Step 3 配置下沉后统一），此处按「无字段 = 无条件输入」处理。
    cond_dirs = getattr(dc, "cond_dirs", ()) or ()
